...
The dataset is the same one we used in session 2b! If you already downloaded it for that session, no need to do so again. 
...
We can also activate the environment using the Anaconda Navigator by selecting the ***Environments*** tab on the left and clicking on the environment you want to activate. 
...
If a package is not available via conda it might be available via pip. However, when working with conda environments, is best practice to use conda install where possible, and only use pip install when necessary.
</details>
...
For our logistic regression, the diagnosis column, which is our outcome of interest, should be 0, 1 not B, M. To fix this, we can use a *dictionary* and `.map()`. We could also use a lambda function as we did in session 2b, however using a dictionary can be more convenient when you need to recode more than 2 vairables. 
