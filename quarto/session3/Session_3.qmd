...
[Cancer Dataset](https://github.com/PythonForRUsers/Python-Workshop.github.io/tree/main/FollowAlong)

## Loading the Dataset{.scrollable}
```{python}
#| class: fragment
...
## Viewing Basic Information {.scrollable}
...
## Row Selection and Column Selection with loc and iloc {.scrollable}
...
## Adding and Modifying Columns {.scrollable}
...
## Reordering Columns to Move a Column to the End {.scrollable}
...
## Method Chaining in Pandas {.scrollable}
...
## Applying Functions in Pandas {.scrollable}
...
## Applying Multiple Conditions with .apply() {.scrollable}
...
## To export a Pandas dataframe to CSV or XLSX {.scrollable}
```{python}
...
## Missing values in Python {.scrollable}
...
## Missing Values in NumPy: `np.nan` {.scrollable}
...
## Missing Values in Pandas: `pd.NA` {.scrollable}
...
## Why use pd.NA in Pandas datasets? {.scrollable}
...
- **Improved Missing Data Semantics**
  ...
  - For example, comparing pd.NA with any value (including itself) results in <NA> rather than a boolean
...
## Comparing `None`, `np.nan`, and `pd.NA` {.scrollable}
...
## Handling Missing Values in Pandas {.scrollable}
...
#| class: fragment
#| output-location: fragment
df["A"]=df["A"].fillna(pd.NA).astype('Int64')  # Replaces missing values with pd.NA, and then changes the column type to 'FLoat64, pandas' nullable float datatype.
...
## Why does casting the column as int64 fail with missing values? {.scrollable}
...
## Best Practices for Missing Data{.scrollable}
...
## Other Pandas Nullable Data Types {.scrollable}
...
## More on missing values {.scrollable}
...
## great_tables for table generation {.scrollable}
...
## Load and Inspect the Data {.scrollable}
...
## Preview of dataset using great_tables {.scrollable}
```{python}
#| class: fragment
#| output-location: fragment
...
## Enhance the Table with Styling {.scrollable}
...
## Group Statistics by Diagnosis {.scrollable}
...
### Download the Session 3 follow-along link here 👇

<br>

[Session 3 Follow-along](https://github.com/PythonForRUsers/Python-Workshop.github.io/tree/main/FollowAlong)
