...
* Pipes (`|>`, `%>%`) are central to chaining operations
* Excels at:

  * clean model outputs and tables
  * concise statistical syntax
...
* In Python, <strong style="color:#007acc">everything is an object</strong>:

  * lists, strings, dataframes, models
* Many things that look like functions in R are **methods attached to objects**
...
Python’s broader ecosystem and flexibility make it the go-to language in areas like **AI, bioinformatics, data engineering, and computational biology**.

> 💡 You’ve already been using this style in Sessions 2 and 3 — creating objects (lists, DataFrames) and calling methods like `.append()` or `.sort_values()`.
> Understanding this object-oriented mindset is **key** to using Python effectively.

> **Note:** Tools like `reticulate` and `rpy2` allow R and Python to coexist in the same project, but that’s beyond the scope of this course.
...
* Learned parameters (coefficients, weights, layers) are stored as **attributes**

This makes model behavior **consistent** between model classes and across libraries. It also **simplifies** saving, loading, and reusing models: both the architecture and learned state live in one object with an expected interface.

> Instead of having a separate results object (as in R), you often access results via attributes or methods on the model object itself.

</div>

<aside class="notes">
In Python, most modeling libraries treat models as objects — not as standalone functions that return a results object like in R.
That means when you create a model, you’re instantiating an object that bundles together both the code that defines the model and all of the data that the model learns during training.

For example, you’ll see methods like .fit(), .predict(), and .score() attached directly to the model object itself. After training, internal details like coefficients or learned weights live in attributes like .coef_ or .classes_.

This leads to a consistent interface across many different model types and libraries. It also means you can save or reload a trained model – architecture and learned state are wrapped in one place.

This design is very different from R’s traditional pattern where a modeling function returns a separate results object. In Python, the model object is both the interface and the container for results.
</aside>



## OOP in Practice: Consistent Interfaces

<div class="clean-text">

Many python modeling packages follow a **predictable OOP API pattern**:

* `.fit(X, y)` — train the model
* `.predict(X)` — generate predictions
* `.score(X, y)` — compute accuracy/metric
* Attributes like `.classes_`, `.coef_`, `.feature_importances_` store learned model state

This consistency makes it easy to **swap models** with minimal code changes.

**Examples of Python modeling libraries:**

* `scikit-learn` — general machine learning (classification, regression)
* `xgboost` — gradient boosting models
* `scikit-survival` — survival models built on sklearn
* `statsmodels` — statistical models with R-like outputs
* `scvi-tools` — single-cell VAE models
* `PyTorch` / `TensorFlow` — neural network frameworks where you define your own model classes

> 💡 To work effectively in Python modeling, think of models as objects whose methods *do work* and whose attributes *store* what the model has learned.

</div>

<aside class="notes">
Because Python models follow this object-oriented pattern, you get a predictable set of methods across very different modeling libraries. Almost all classifiers or regressors have .fit(), .predict(), and often .score().

Attributes like .classes_, .coef_, and .feature_importances_ hold learned data, and you access them directly on the model instance after training. This consistency makes it easy to change models without changing much code.

Let’s look at some common libraries you might encounter:

scikit-learn, which we’ll focus on today, provides this consistent API for many classic machine learning models.

xgboost uses the same pattern for gradient boosted trees.

scikit-survival extends this to survival analysis.

statsmodels provides statistical models with a syntax closer to R, but many still follow an object-oriented structure.

For deep learning, frameworks like PyTorch and TensorFlow require you to define your own model classes, but the basic idea – objects with methods and attributes – is the same.

This list is here so you can explore these packages on your own later.
...
##  Core OOP Principles in Python Modeling

<div class="clean-text">

Python’s object-oriented design builds on a few key principles:

1. **Encapsulation** — Data and behaviors live together

   * e.g., a `StandardScaler` stores means/variances and has `.fit()`/`.transform()`

2. **Inheritance** — New classes reuse existing functionality

   * e.g., many sklearn models share base classes like `BaseEstimator`

3. **Abstraction** — Same interface hides complexity

   * e.g., calling `.fit()` looks the same regardless of model internals

4. **Polymorphism (Duck Typing)** — Different objects with the same methods can be used interchangeably

   * e.g., any classifier with `.fit()` and `.predict()` can be used in a pipeline

**Classes and objects**

* A **class** is a blueprint for objects
* An **object** (instance) has attributes (data) and methods (behaviors)
* Use `type(obj)` and `isinstance(obj, Class)` to inspect objects in code

</div>

<aside class="notes">
Now that we see why modeling frameworks use objects, let’s connect that to some core object-oriented principles in Python. These aren’t unique to modeling, but they help explain why everything feels this way in Python.

First is encapsulation — bundling data and behavior together. A StandardScaler, for instance, stores feature means and variances and has .fit() and .transform() methods.

Second is inheritance. Many sklearn models reuse code from base classes like BaseEstimator. This lets new models inherit common behavior without rewriting everything.

Third is abstraction. From the outside, calling .fit() looks the same whether you’re working with a random forest or a neural network. The internal details are hidden behind a common interface.

Finally, polymorphism, or duck typing in Python. If different objects implement the same method names, we can use them interchangeably. That’s why we can write code that works with any classifier that supports .fit() and .predict() — even if under the hood they do very different things.

Briefly, what are classes and objects? A class is like a blueprint — for example, a model class defines what a model can do. An object is an instance of that blueprint, with its own stored data and methods. In code, you can inspect these using type() and isinstance().
</aside>
...
<ul>
 <li>When you create a model, you’re **instantiating an object** of a predefined class (e.g., `LogisticRegression()`).</li>    
...
For our neural network classifier, the model is **supervised** (meaning it is dependent on the outcome 'y' data). This time, we need to split our data into a training and test set. <br> 
...
# Convert numpy arrays to torch tensors (for PyTorch)
...
y_train_t = torch.tensor(y_encoded[y_train.index], dtype=torch.long)
y_test_t  = torch.tensor(y_encoded[y_test.index], dtype=torch.long)
```
...
For large datasets, batching is necessary. 
...
Because we are using PyTorch here, we need to build the classifier ourselves. 
...
**Parameters**: Set by user at time of instantiation    
-number of hidden layers, number of classes  <br>
...
To turn PenguinNet into a scikit-learn-style classifier, we have to define the `.fit()` and `.predict()` methods ourselves. 
...
<aside class="notes">
To make an Sklearn-style classifier, we need to define the firt, score, predict methods ourselves. This is by design: Neural network arctitectures can be complex and have custom activation functions that would make it difficult to have pre-packaged methods. However, it is useful for us as programmers to create these methods and the wrapper Classifier class for our own use. 
...
```{python}
#| output: true
penguin_nn = PenguinNetClassifier(
    hidden_units=32,
    lr=0.01,
    epochs=300,
    batch_size=32
...
penguin_nn.fit(X_train, y_train)
```
...
<div class="clean-text">
The homework for this session can be found at "H:\Biostatistics\PythonWorkshop\Homework Exercises\session4_homework_blank.ipynb" <br>
