...
<a href="session4/session4v3slides.html">slides link</a>
...
<aside class="notes">
These are some links to tutorials for the packages. 
</aside>
...
Of course we can use the speak() and repr() methods that we defined for the dog class. We can also use the training method that we just defined. 
...
A mixin class can add both attributes and methods, and it can potentially work with multiple base classes. 
</aside>
...
However, it is beneficial for us to design our Classifier wrapper to match the scikit-learn API, which will allow us to take advantage of Python's ducktyping later on. We also inherit from sklearns BaseEstimator and ClassifierMixin to provide our classifer with the sklearn `score` method
</aside>
