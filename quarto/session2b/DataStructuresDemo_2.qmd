...
### Aside: Creating folders for project housekeeping
...
While we use piping in R (%>%) to chain together a bunch of commands, in python we put the whole set of commands in parentheses and then can chain together methods on different lines. 
...
The <a href = "https://posit-dev.github.io/great-tables/articles/intro.html">great_tables</a> package is designed to be similar to `gt()` in R and allows us to make publication-ready tables in python. 

If we want to make a simple table showing the counts for the tumor size categories, we can just use the GT function to make a table. 
...
If we want to make a more complex summary table, we can get summary statistics for the various columns of our dataset and use methods from the great_tables package to handle table formatting. 
